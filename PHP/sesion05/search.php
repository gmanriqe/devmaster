<?php
    ...
    //Recuerda: que no declaras la variable $productos dentro de la clase ConsultaProducotos, ya que apartir que una variable tiene metodos get,set o get y set dentro de una clase, todas las variables declaras al menos deben tener un set o get declarados.
    class ConsultaProductos extends ListaProductos {
        var $codigo;
        var $resultado;

        //Recuerda: set y get son constructores.
        ...
        function setResultado($value){
            ...
            /*ejemplo del llenado del arreglo
            $resultado[]=6
            $resultado[]=Mermelada de grosellas de la abuela
            $resultado[]=Grandma Kellys Homestead
            $resultado[]=Condimentos
            $resultado[]=12 - frascos 8 l
            $resultado[]=25
            $resultado[]=120*/
            //Aquí $resultado tendra ya todos los elementos con su posicion, en el orden que fue llenado.
            //echo $this->resultado;
        ...
        public function buscar() {
            ...
            foreach ($this->productos as $key => $value) {
                if($value[0] == $this->codigo){
                    foreach ($value as $subkey => $subvalue) {
                        // echo $subvalue."<br>";
                        $this->setResultado($subvalue);
                    }
                }
            }
        }
    ...
    $busqueda = new ConsultaProductos;
    $busqueda->setCodigo($_POST["codigo"]);
    $busqueda->buscar();

    $arraybusqueda = $busqueda -> getResultado();
